...
FROM
  ...
  (VALUES ('2010/11/03 06:30 PM', 'Navegantes', 'Leones', 'Universitario')) AS Data ("inicio", "equipo_local", "equipo_visitante", "estadio")
WHERE
...
$r = $database->insert($table, $columns, $values);
...
FROM
  ...
  "Fantasy"."Equipo" AS Equipo_visitante,
  "Fantasy"."Estadio" AS Estadio,
  (VALUES ('2010/11/03 06:30 PM', 'Navegantes', 'Tigres', 'Universitario')) AS Data ("inicio", "equipo_local", "equipo_visitante", "estadio")
WHERE
   Data.equipo_local = Equipo_local."nombre corto" AND
   Data.equipo_visitante = Equipo_visitante."nombre corto" AND
   Data.estadio = Estadio.nombre;
EOD;
  ...
  $output = $database->insert($table, $columns, $values);
  ...
  if (!$output) {
    ...
    echo "(i) El juego se ha insertado correctamente.";
